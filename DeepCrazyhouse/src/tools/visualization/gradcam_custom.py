...
def _get_grad(net, image, class_id=None, conv_layer_name=None, image_grad=False):
    ...
    if class_id == None:
        ...
        model_output = out[1][0].asnumpy()
        class_id = np.argmax(model_output)

    # Create a one-hot target with class_id and backprop with the created target
    one_hot_target = mx.nd.one_hot(mx.nd.array([class_id]), yp_test.shape[1]) #1000)
#    out.backward(one_hot_target, train_mode=False)
    out[1].backward(one_hot_target, train_mode=False)
    #out[0][1].backward(one_hot_target, train_mode=False)
...
def visualize_grad(net, preprocessed_img, orig_img, conv_layer_name):
