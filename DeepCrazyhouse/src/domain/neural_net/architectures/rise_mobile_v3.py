...
def preact_residual_dmixconv_block(data, channels, channels_operating, name, kernels=None, act_type='relu', use_se=True):
    ...
    conv1 = mx.sym.Convolution(data=bn1, num_filter=channels, kernel=1, pad=0, no_bias=True, name=name + '_conv1')
    ...
    out = get_act(data=bn2, act_type=act_type, name=name + '_act2')
    if use_se:
        out = channel_squeeze_excitation(out, channels, name=name + '_se', ratio=4, act_type=act_type)
    con3 = mx.sym.Convolution(data=out, num_filter=channels, kernel=1,
                               pad=0, num_group=1,
                               no_bias=True, name=name + '_conv3')
    bn2 = mx.sym.BatchNorm(data=con3, name=name + '_bn2')
...
def value_head(data, channels_value_head=256, value_kernelsize=1, act_type='relu', value_fc_size=256,
    ...
    if use_se:
        value_out = channel_squeeze_excitation(value_out, channels_value_head, name='value_se', ratio=4, act_type=act_type)
