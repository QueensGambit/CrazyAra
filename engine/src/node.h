        ...
        if (d->childNumberVisits[childIdx] == searchSettings->virtualLoss) {
            ...
            if(isMaxOperator) {
                ...
                if (d->childNodes[childIdx] != nullptr) {
                    float mean_value = d->childNodes[childIdx]->valueSum / d->childNodes[childIdx]->realVisitsSum;
                    maxValue = max(mean_value , -score_child_qValue_max(get_child_node(childIdx), searchSettings));
