...
void MCTSAgent::set_root_node_predictions()
{
    ...
    GamePhase current_phase = state->get_phase();
    nets[current_phase]->predict(inputPlanes, valueOutputs, probOutputs, auxiliaryOutputs);
    size_t tbHits = 0;
    fill_nn_results(0, nets[current_phase]->is_policy_map(), valueOutputs, probOutputs, auxiliaryOutputs, rootNode.get(), tbHits,
